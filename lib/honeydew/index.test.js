...
test('when heartbeat() is called && worker.idle, call worker.findTask().execute()', t => {
    const task = td.function();
    const taskFinder = td.function();
    td.when(taskFinder()).thenResolve(task);
    td.when(task()).thenResolve();
    const worker = new Worker(taskFinder);
    worker.sleep();
    return worker.heartbeat().then(() => t.pass());
});
test('if findTask() returns a promise & findTask().then(task => task()) completes before worker.patience elapses, goIdle is called with no params', t => {
    const task = td.function();
    const taskFinder = td.function();
    td.when(taskFinder()).thenResolve(task);
    td.when(task()).thenResolve();
    ...
    return worker
        .heartbeat()
        .then(() => t.notThrows(() => td.verify(worker.goIdle())));
});
test('if findTask().then(task => task()) does not complete before worker.patience elapses, goIdle is called with timeout Error', t => {
...
test('if findTask() does not return a promise, promise wrapper still works', t => {
    const taskFinder = () => () => true;
    const worker = new Worker(taskFinder);
    worker.sleep();
    worker.goIdle = td.function();
    return worker
